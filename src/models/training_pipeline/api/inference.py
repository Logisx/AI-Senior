...
from src.models.training_pipeline import constants, models
from src.models.training_pipeline.data import qa
from src.models.training_pipeline.prompt_templates.prompter import get_llm_template
...
class InferenceAPI:
    ...
    def from_config(
        cls,
        config: ConfigurationManagement.get_inference_params(),
        root_dataset_dir: Path,
        model_cache_dir: Path = constants.CACHE_DIR,
    ):
        """
        ...
        return cls(
            ...
            template_name=config.model_template_name,
            root_dataset_dir=root_dataset_dir,
            test_dataset_file=config.dataset_file,
            max_new_tokens=config.model_max_new_tokens,
            temperature=config.model_temperature,
            model_cache_dir=model_cache_dir,
            debug=config.get("setup_debug", False),
            device=config.get("setup_device", "cuda:0"),
        )
    ...
    def infer_all(self, output_file: Optional[Path] = None) -> None:
        ...
        if should_save_output:
            FileManagement.save_to_json(prompt_and_answers, output_file)
